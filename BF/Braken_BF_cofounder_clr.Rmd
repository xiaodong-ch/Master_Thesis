...
iphy <- iphy %>% microbiome::transform(transform = "clr")

saveRDS(iphy, "Braken&BF_cof_clr-2021-12-14.rds")
```
  ...
  for (variable in colnames(sample)) {
    ...
    if (nrow(unique(curr_meta[variable])) > 1) {
      ...
      curr_dist <- curr_asv[rowSums(curr_asv) != 0,] %>%
        t() %>%
        vegdist(method = "euclidean") ##give the distance between two different sample group
...
write_csv(all_adonis, "adonis_BF_cof_clr-2021-12-14.csv")
write_csv(list_beta_df, "betadispersion_BF_cof_clr-2021-12-14.csv")
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
all_adonis <- read_csv("adonis_BF_cof_clr-2021-12-14.csv")
list_beta_df <- read_csv("betadispersion_BF_cof_clr-2021-12-14.csv")
iphy <- read_rds("Braken&BF_cof_clr-2021-12-14.rds")
```
...
write_csv(sig_adonis, "sig_adonis_BF_cof_clr-2021-12-14.csv")
```
