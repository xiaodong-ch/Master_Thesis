...
read in data:
```{r}
vocabulary <- read.csv("../Background Variable/newvocabulary.csv")
WGS_Background_all <- read.csv("../Background Variable/WGS_Background_all_v1.csv")
...
sample <- braken_infant_sample %>% left_join(WGS_Background_all, by = "Family_ID") %>% 
...
saveRDS(iphy, "Braken_hum&BVs_01.11.21.rds")
```
...
write_csv(all_adonis, "adonis_backgound2021-07-20.csv")
write_csv(list_beta_df, "betadispersion2021-07-20.csv")
```
...
all_adonis <- read_csv("adonis_backgound2021-07-20.csv")
list_beta_df <- read_csv("betadispersion2021-07-20.csv")
iphy <- read_rds("Braken_hum&BVs_01.11.21.rds")
```
...
for (variable in unique(sig_adonis$variable)) {
  for (age in unique(sig_adonis$sample_type)) {
    if (nrow(sig_adonis[sig_adonis$variable == variable &
                        sig_adonis$sample_type == age,]) > 0) {
      sig_status <- sig_adonis[sig_adonis$variable == variable &
...
write_csv(all_pair_adonis, "pairwise_2021-10-04.csv")
```
